# Data Structures and Algorithm Analysis in C++

# Contents
- [Data Structures and Algorithm Analysis in C++](#data-structures-and-algorithm-analysis-in-c)
- [Contents](#contents)
  - [Chapter 1: Programming: A general overview](#chapter-1-programming-a-general-overview)
    - [1.1 What's this book about ?](#11-whats-this-book-about-)
    - [1.2 Mathermatics review](#12-mathermatics-review)
    - [1.3 A brief introduction to recursion](#13-a-brief-introduction-to-recursion)
    - [1.4 C++ Classes](#14-c-classes)
    - [1.5 C++ Details](#15-c-details)
    - [1.6 Templates](#16-templates)
    - [1.7 Using matrices](#17-using-matrices)

## Chapter 1: Programming: A general overview
(Cuốn sách này mình tập trung vào coding. Các lý thuyết mình sẽ chỉ viết tốc kí các nội dung mình đang cần nên sẽ không đầy đủ)

### 1.1 What's this book about ?

### 1.2 Mathermatics review

### 1.3 A brief introduction to recursion

### 1.4 C++ Classes

### 1.5 C++ Details

### 1.6 Templates

### 1.7 Using matrices

